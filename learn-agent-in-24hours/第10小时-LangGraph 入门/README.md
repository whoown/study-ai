...
- `Node`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Edge`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `State`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Reducer`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
...
- `calculator`：承担本章流程中的一个明确职责，阅读时要关注它的输入、输出和在整条链路中的位置。
- `build_llm`：负责组装对象、提示词、索引、图或应用实例。
- `call_model`：负责真正与模型交互，并把结果交回主流程。
- `build_graph`：定义节点、边和路由关系，是图编排的关键入口。
- `simulate_without_api`：无 API Key 时的教学降级分支。
- `run_demo`：本章最短可运行演示入口。
- `main`：脚本入口，通常只负责启动案例。
