...
- `Role`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Pipeline`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Hand-off`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Coordination`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
...
- `maybe_llm_role`：承担本章流程中的一个明确职责，阅读时要关注它的输入、输出和在整条链路中的位置。
- `planner_agent`：负责先把任务拆清楚。
- `researcher_agent`：负责收集事实和素材。
- `writer_agent`：负责把素材组织成对用户可读的输出。
- `run_pipeline`：驱动一段完整流程，把前面准备好的零件真正串起来。
- `main`：脚本入口，通常只负责启动案例。
