...
- `Agent`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Tool`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Loop`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
...
- `fake_brain`：用规则模拟模型决策，帮助你先看懂 Agent 的骨架。
- `run_episode`：驱动一段完整流程，把前面准备好的零件真正串起来。
- `main`：脚本入口，通常只负责启动案例。
