...
- `App Factory`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Mock/Real`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Deployment`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
...
- `_answer_with_llm`：内部辅助函数，为主流程提供局部能力。
- `_answer_mock`：内部辅助函数，为主流程提供局部能力。
- `create_app`：应用工厂函数，负责组装 API 服务。
- `main`：脚本入口，通常只负责启动案例。
