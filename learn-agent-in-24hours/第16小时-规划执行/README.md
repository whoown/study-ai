...
- `Planning`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Dependency`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
- `Execution`：这是本章必须抓住的关键词，建议你运行代码时带着它去观察程序行为。
...
- `build_plan`：把目标拆成可执行步骤。
- `can_run_step`：检查某步是否满足执行条件。
- `fake_tool_call`：承担本章流程中的一个明确职责，阅读时要关注它的输入、输出和在整条链路中的位置。
- `run_plan_and_execute`：驱动一段完整流程，把前面准备好的零件真正串起来。
- `main`：脚本入口，通常只负责启动案例。
